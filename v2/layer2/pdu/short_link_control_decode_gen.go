...
// DecodeShortLCCSysParms decodes a ShortLCCSysParms per ETSI TS 102 361-4 §7.1.2 C_SYS_Parms Short LC PDU
func DecodeShortLCCSysParms(data [24]bit.Bit) (ShortLCCSysParms, fec.FECResult) {
	var result ShortLCCSysParms
	var fecResult fec.FECResult
	result.MODEL = bit.BitsToUint8(data[:], 0, 2)
	copy(result.NetSiteRaw[:], data[2:14])
	result.Reg = bit.BitsToBool(data[:], 14)
	result.CommonSlotCounter = bit.BitsToUint16(data[:], 15, 9)
	return result, fecResult
}

// EncodeShortLCCSysParms encodes a ShortLCCSysParms per ETSI TS 102 361-4 §7.1.2 C_SYS_Parms Short LC PDU
func EncodeShortLCCSysParms(s *ShortLCCSysParms) [24]bit.Bit {
	var data [24]bit.Bit
	copy(data[0:2], bit.BitsFromUint8(s.MODEL, 2))
	copy(data[2:14], s.NetSiteRaw[:])
	if s.Reg {
		data[14] = 1
	}
	copy(data[15:24], bit.BitsFromUint16(s.CommonSlotCounter, 9))
	return data
}

func (s *ShortLCCSysParms) ToString() string {
	return fmt.Sprintf("ShortLCCSysParms{ MODEL: %d, NetSiteRaw: %v, Reg: %t, CommonSlotCounter: %d }", s.MODEL, s.NetSiteRaw, s.Reg, s.CommonSlotCounter)
}

// DecodeShortLCPSysParms decodes a ShortLCPSysParms per ETSI TS 102 361-4 §7.1.2 P_SYS_Parms Short LC PDU
func DecodeShortLCPSysParms(data [24]bit.Bit) (ShortLCPSysParms, fec.FECResult) {
	var result ShortLCPSysParms
	var fecResult fec.FECResult
	result.MODEL = bit.BitsToUint8(data[:], 0, 2)
	copy(result.NetSiteRaw[:], data[2:14])
	result.PayloadChannelType = bit.BitsToBool(data[:], 14)
	result.CommonSlotCounter = bit.BitsToUint16(data[:], 15, 9)
	return result, fecResult
}

// EncodeShortLCPSysParms encodes a ShortLCPSysParms per ETSI TS 102 361-4 §7.1.2 P_SYS_Parms Short LC PDU
func EncodeShortLCPSysParms(s *ShortLCPSysParms) [24]bit.Bit {
	var data [24]bit.Bit
	copy(data[0:2], bit.BitsFromUint8(s.MODEL, 2))
	copy(data[2:14], s.NetSiteRaw[:])
	if s.PayloadChannelType {
		data[14] = 1
	}
	copy(data[15:24], bit.BitsFromUint16(s.CommonSlotCounter, 9))
	return data
...
func (s *ShortLCPSysParms) ToString() string {
	return fmt.Sprintf("ShortLCPSysParms{ MODEL: %d, NetSiteRaw: %v, PayloadChannelType: %t, CommonSlotCounter: %d }", s.MODEL, s.NetSiteRaw, s.PayloadChannelType, s.CommonSlotCounter)
}
...
func DecodeShortLC(data [36]bit.Bit) (ShortLC, fec.FECResult) {
	...
	case enums.SLCOCSysParms:
		_decoded, _ := DecodeShortLCCSysParms(_dispatchBits)
		result.CSysParms = &_decoded
	case enums.SLCOPSysParms:
		_decoded, _ := DecodeShortLCPSysParms(_dispatchBits)
		result.PSysParms = &_decoded
	}
	return result, fecResult
}
...
func EncodeShortLC(s *ShortLC) [36]bit.Bit {
	...
	case s.ActivityUpdate != nil:
		...
		copy(data[4:28], _pduBits[:])
	case s.CSysParms != nil:
		_pduBits := EncodeShortLCCSysParms(s.CSysParms)
		copy(data[4:28], _pduBits[:])
	case s.PSysParms != nil:
		_pduBits := EncodeShortLCPSysParms(s.PSysParms)
		copy(data[4:28], _pduBits[:])
	}
...
func (s *ShortLC) ToString() string {
	...
	case s.CSysParms != nil:
		_ret += s.CSysParms.ToString()
	case s.PSysParms != nil:
		_ret += s.PSysParms.ToString()
	}
	_ret += " }"
	return _ret
}
