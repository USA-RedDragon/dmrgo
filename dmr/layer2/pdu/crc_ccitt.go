...
//
// Consider the 80 data header bits as the coefficients of a polynomial M(x) of degree 79, associating the MSB of the
// zero-th header octet with x79 and the LSB of the ninth header octet with x0 . Define the generator polynomial, GH(x),
// and the inversion polynomial, IH(x).
//
// (18) GH(x) = x16 + x12 + x5 + 1
// (19) IH(x) = x15 + x14 + x13 + ... + x2 + x + 1
//
// The header CRC polynomial, FH(x), is then computed from the formula:
//
// (20) FH(x) = ( x16 M(x) mod GH(x) ) + IH(x)
//
...
// x15 and the LSB of the next octet of the CRC corresponding to x0 .

var CCITT16_TABLE2 [256]uint16 = [256]uint16{
...
func CalculateCRCCCITT(data []byte) uint16 {
	...
	for _, b := range data {
		crc16 = (uint16(byte(crc16)) << 8) ^ CCITT16_TABLE2[byte(crc16>>8)^b]
	}
